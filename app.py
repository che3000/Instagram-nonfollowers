...
# -*- coding: utf-8 -*-
"""
Instagram Non-Followers Web Application.

This Flask-based web application allows users to analyze their Instagram followers
and following lists to identify non-followers and fans they don't follow back.
Includes session management, rate limiting, and result caching.
"""
...
def generate_plotly_charts(following_count, followers_count, following_only_count, fans_only_count):
    ...
    try:
        ...
        import plotly.io as pio  # type: ignore  # pylint: disable=import-outside-toplevel
        ...
        fig.update_layout(
            ...
            annotations=[
                dict(text="你追蹤的人", x=0.24, y=-0.15, font_size=16,
                     showarrow=False, font_color='white'),
                dict(text="追蹤你的人", x=0.76, y=-0.15, font_size=16,
                     showarrow=False, font_color='white')
            ]
        )
...
def sse(data: str):
    ...
    Args:
      data (str): The data to be sent as an SSE message.

    Returns:
      str: A formatted SSE message string with the data field followed by two newlines.
    """
    return f"data: {data}\n\n"


# 用於追蹤 log_emit 的狀態
_log_state = {"last_same_line": False}


def log_emit(msg: str, same_line: bool = False):
    """
    Emit a log message to both server terminal and frontend via SSE.
    This function outputs log messages to the server's terminal for monitoring
    (useful in Docker/host environments) and simultaneously sends them to the
    frontend via Server-Sent Events (SSE).
    Args:
      msg (str): The log message to emit.
      same_line (bool, optional): If True, overwrites the current line using
        carriage return (\r) for progress updates. If False, prints on a new
        line. Defaults to False.
    Returns:
      The result of sse("LOG:" + msg), which sends the log message to the frontend.
    Note:
      - Uses a global _log_state dictionary to track whether the last log was
        on the same line to ensure proper line breaks.
      - When same_line=True, pads the message to 80 characters to clear previous
        content and prevent residual text.
    """
    ...
    if same_line:
        ...
        # 如果前一行是同行更新，先換行
        if _log_state["last_same_line"]:
            print()  # 換行
        print(msg, flush=True)

    _log_state["last_same_line"] = same_line
...
def to_user_obj(user, include_avatar: bool = True) -> Dict[str, str]:
    """
    Convert an Instagram user object to a dictionary containing user information.

    This function extracts user data from an instaloader user node to avoid making
    individual Profile requests, which saves time and bandwidth.

    Args:
      user: An instaloader user node object containing user information.
      include_avatar (bool, optional): Whether to include the avatar URL in the result.
        Defaults to True.

    Returns:
      Dict[str, str]: A dictionary containing:
        - username (str): The user's username
        - full_name (str): The user's full name (empty string if not available)
        - avatar_url (str): The user's profile picture URL (empty string if not
          available or if include_avatar is False). High-quality URLs are
          automatically converted to standard quality to reduce request overhead.

    Note:
      Avatar URLs are automatically downgraded from high-quality versions
      (s150x150, s320x320) to standard quality (s100x100) to minimize bandwidth usage.
    """
...
def fetch_users_with_progress(iterable, total: Optional[int], label: str,
                              include_avatar: bool = True):
    """Fetch users with progress tracking and error handling.

    Args:
        iterable: Iterator of user objects from Instagram API
        total: Total number of users expected (or None if unknown)
        label: Label for progress messages
        include_avatar: Whether to include avatar URLs
    """
    ...
    while True:
        try:
            user = next(iterator)
            users_pairs.append(
                (user.username, (getattr(user, "full_name", "") or "")))
...
@APP.post("/start")
def start():
    """Initialize a new Instagram analysis session.
    ...
    print(
        f"[DEBUG] Start request - username: {username}, has_password: {bool(password)}", flush=True)
    ...
    RUNS[username] = {"password": password, "twofa_code": None,
                      "running": False, "fetch_avatar": fetch_avatar}
    print(
        f"[DEBUG] Added {username} to RUNS. Current RUNS: {list(RUNS.keys())}", flush=True)
    return {"ok": True}
                    ...
                    if len(datetime_str) == 14 and datetime_str.isdigit():
                        try:
                            ...
                            if all_files_exist:
                                ...
                                print(
                                    f"[DEBUG] 找到有效的結果資料夾: {item} "
                                    f"(IGID: {igid}, 日期時間: {datetime_str})", flush=True)
                            else:
                                print(
                                    f"[DEBUG] 資料夾 {item} 缺少必要的 CSV 檔案",
                                    flush=True)
...
def read_existing_csv(
    ...
    try:
        ...
        return result
    except (OSError, IOError) as e:  # More specific exceptions
...
def check_session(skip_folders: bool = False):
    ...
    """
    print(
        f"[DEBUG] check_session called with skip_folders={skip_folders}", flush=True)
...
def handle_check_session():
    """Check for existing sessions and result folders.

    Returns:
        JSON response with session/folder information
    """
...
@APP.get("/stream")
def stream():
    """Stream Instagram analysis progress via Server-Sent Events.
    ...
    print(
        f"[DEBUG] Stream request - username: {username}, use_existing: {use_existing}", flush=True)
    ...
    if use_existing:
        ...
        fetch_avatar_value = (
            fetch_avatar_override if fetch_avatar_override is not None else True
        )
        RUNS[username] = {
            "password": None, "twofa_code": None, "running": True,
            "fetch_avatar": fetch_avatar_value
        }
    elif username not in RUNS:
        print(
            f"[DEBUG] Username {username} not in RUNS, returning error", flush=True)
    ...
    def run_and_stream():
        ...
        try:
            ...
            yield log_emit(f"[INFO] 當前時區: {tz_info}, 本機時間: {time_str}")

            loader = Instaloader()
            loader.context.iphone_support = False
            if not fetch_avatar:
            ...
            loader.context.request_timeout = 120  # 增加超時時間

            # 安全地設定速率控制參數
            try:
                # pylint: disable=protected-access
                if hasattr(loader.context, '_rate_controller') and loader.context._rate_controller:
                    # 保存原始方法
                    original_query_waittime = loader.context._rate_controller.query_waittime

                    # 創建新的方法來覆蓋原始等待時間
                    def custom_query_waittime(query_type, current_time, untracked_queries):
                        if callable(original_query_waittime):
                            base_wait = original_query_waittime(
                    ...
                    # 替換方法
                    # pylint: disable=protected-access
                    loader.context._rate_controller.query_waittime = custom_query_waittime
                ...
                while True:
                    try:
                        ...
                        if any(k in msg for k in ("challenge", "checkpoint", "fail")):
                            ...
                            yield sse("UNLOCK_FORM")
                            yield log_emit(
                                "[INFO] 請重新檢查帳號密碼是否正確，或到 Instagram App → "
                                "安全性 / 登入活動 → 允許這次登入")
            ...
            try:
                ...
                while wait_time < max_wait:
                    if not result_queue.empty():
                        ...
                        if wait_time == 10:
                            ...
                            yield log_emit(
                                "[RATE-LIMIT] 檢測到 API 限制 (429 Too Many Requests)")
                            yield log_emit(
                                "[INFO] Instagram 要求等待後重試，通常為 15-30 分鐘")
                            ...
                            retry_time_str = estimated_retry_time.strftime(
                                '%H:%M')
                            yield log_emit(
                                f"[INFO] 預估重試時間：{retry_time_str} (台北時間)")
                        elif wait_time == 120:
                ...
                wait_match = re.search(
                    r'retry.+?(\d+)\s*minutes?', error_msg, re.IGNORECASE)
                if wait_match:
                    ...
                    yield log_emit(
                        f"[INFO] 預計重試時間：{retry_time.strftime('%H:%M')} (台北時間)")
                    yield log_emit("[建議] 請關閉所有 Instagram App，並等待指定時間後再試")
                else:
            ...
            # following
            try:
                following_count = profile.followees if hasattr(
                    profile, 'followees') else None
                following_pairs, following_objs = yield from (
                    fetch_users_with_progress(
                        profile.get_followees(), following_count, "following",
                        include_avatar=fetch_avatar
                    )
                )
            except Exception as e:  # pylint: disable=broad-except
            ...
            # followers
            try:
                followers_count = profile.followers if hasattr(
                    profile, 'followers') else None
                followers_pairs, followers_objs = yield from (
                    fetch_users_with_progress(
                        profile.get_followers(), followers_count, "followers",
                        include_avatar=fetch_avatar
                    )
                )
            except Exception as e:  # pylint: disable=broad-except
            ...
            following_only_objs = filter_objs(
                following_objs, following_only_set)
...
def download(filename):
    ...
    Args:
        filename: Path to the CSV file (may include folder structure)

    Returns:
        CSV file download or error message
    """
    # 支援資料夾結構的檔案下載
...
def generate_chart():
    ...
    chart_data = generate_plotly_charts(
        following, followers, following_only, fans_only)
...
def load_existing():
    ...
    if folder and igid and date:
        # 驗證 folder 值:僅允許預期資料夾名稱格式(數字、字母、底線、點號、連字號),防止路徑跳脫攻擊
        import re  # pylint: disable=import-outside-toplevel
